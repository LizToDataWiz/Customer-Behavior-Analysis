...
-- Create a user summary table
WITH user_summary AS (
    SELECT
        user_id,
        COUNT(user_id) AS user_count,
        CASE 
            WHEN age BETWEEN 13 AND 17 THEN 'Teenagers' -- categorize users into age groups.
    ...
    FROM (
        SELECT
            user_id,
            DATE_PART('YEAR', AGE(CURRENT_DATE, birthdate)) AS age -- calculate the age based on birtdate
        FROM users
    ) AS T1
    GROUP BY 1,3
),
-- Calculate abandoned sessions, sessions where no flights or hotels are booked
